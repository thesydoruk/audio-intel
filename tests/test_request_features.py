    ...
    def test_disables_whisper_word_timestamps_when_ctc_will_run(self) -> None:
        ...
        with (
            ...
            with (
                ...
                transcribe_chunks.return_value = (speech, ["hi"], ["en"])
                apply_alignment.return_value = (speech, {"alignment_applied": True})

                transcriber.transcribe("clip.wav", align=True)

                self.assertEqual(transcribe_chunks.call_args.kwargs["word_timestamps"], False)

    def test_keeps_whisper_word_timestamps_without_alignment(self) -> None:
        cfg = replace(load_config(), alignment_enabled=True)
        ...
        with (
            ...
            with (
                ...
                transcriber.transcribe("clip.wav", align=False)

                self.assertEqual(transcribe_chunks.call_args.kwargs["word_timestamps"], True)
