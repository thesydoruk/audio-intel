...
from audio_intel.diarization.chunks import (  # noqa: E402
    ...
    merge_speaker_ids_by_embedding,
    offset_intervals,
    owned_span,
    plan_diarization_chunks,
    remap_segment_speaker_ids,
    remap_speaker_ids,
)
...
class SpeakerLinkingTest(unittest.TestCase):
    ...
    def test_links_speaker_absent_from_overlap_via_gallery(self) -> None:
        """A returning speaker with no overlap speech still remaps via gallery."""
        previous = [
            {"start": 10.0, "end": 40.0, "speaker_id": "spk_0"},
            {"start": 560.0, "end": 580.0, "speaker_id": "spk_1"},
        ]
        local = [
            # Only spk_1 (different person) speaks in the overlap zone.
            {"start": 570.0, "end": 590.0, "speaker_id": "spk_0"},
            # Returning voice appears only after the overlap.
            {"start": 900.0, "end": 920.0, "speaker_id": "spk_1"},
        ]
        voice_a = [1.0, 0.0, 0.0]
        voice_b = [0.0, 1.0, 0.0]
        mapping, next_idx = link_local_speakers(
            ...
            chunk_start_s=570.0,
            overlap_s=30.0,
            local_embeddings={"spk_0": voice_b, "spk_1": voice_a},
            global_embeddings={"spk_0": voice_a, "spk_1": voice_b},
            next_global_index=2,
            threshold=0.75,
        )
        self.assertEqual(mapping["spk_0"], "spk_1")
        self.assertEqual(mapping["spk_1"], "spk_0")
        self.assertEqual(next_idx, 2)

    def test_temporal_fallback_when_embeddings_missing(self) -> None:
        previous = [
            {"start": 560.0, "end": 585.0, "speaker_id": "spk_0"},
        ]
        local = [
            {"start": 570.0, "end": 590.0, "speaker_id": "spk_0"},
        ]
...
class SpeakerEmbeddingMergeTest(unittest.TestCase):
    def test_merges_near_duplicate_embeddings(self) -> None:
        same = [1.0, 0.0, 0.0]
        other = [0.0, 1.0, 0.0]
        mapping = merge_speaker_ids_by_embedding(
            {"spk_0": same, "spk_3": same, "spk_1": other},
            threshold=0.75,
        )
        self.assertEqual(mapping["spk_0"], "spk_0")
        self.assertEqual(mapping["spk_3"], "spk_0")
        self.assertEqual(mapping["spk_1"], "spk_1")

    def test_remap_segment_speaker_ids(self) -> None:
        segments = [
            {"speaker_id": "spk_3", "start": 0.0, "end": 1.0, "text": "a"},
            {"speaker_id": "spk_1", "start": 1.0, "end": 2.0, "text": "b"},
        ]
        remapped = remap_segment_speaker_ids(segments, {"spk_3": "spk_0"})
        self.assertEqual(remapped[0]["speaker_id"], "spk_0")
        self.assertEqual(remapped[1]["speaker_id"], "spk_1")
