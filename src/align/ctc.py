...
class CTCAligner:
    ...
    def align_transcript(
        ...
        *,
        ...
        ``audio`` is a 16 kHz mono float32 window; when set, the file is not loaded.
        Segment times must already be relative to that window.
        """
        try:
            import whisperx
        except ImportError as exc:
    ...
    def _map_aligned_segments(
        ...
        A segment is marked ``alignment_failed`` when aligned words are missing
        or lack ``start`` / ``end`` after ``align()``.
        """
        ...
        for idx, seg in enumerate(aligned_segments):
            ...
            alignment_failed = bool(text) and not words
