...
from audio_intel.audio.decode import SAMPLE_RATE, extract_wav_window
...
class SpeakerDiarizer:
    ...
    def _speaker_embeddings_for_window(
        ...
        *,
        ...
        for sid, spans in spans_by_speaker.items():
            embedding = embedder.embed_speaker_spans_from_path(
                path,
                spans,
    ...
    def _link_chunk_results(
        ...
        *,
        ...
        Keeps a running embedding gallery so speakers who skip an overlap zone
        can still reattach to an earlier global id. Embeddings are read from
        speaker spans on disk (at most 30 s per speaker), not the full chunk.
        """
        global_intervals: list[dict] = []
        chunk_results: list[list[dict]] = []
        global_gallery: dict[str, list[float]] = {}
        next_global_index = 0

        for chunk in chunks:
            global_chunk_intervals = raw_by_index[chunk.index]
            # Embed from speaker spans only (≤30 s each), not the full 10-minute chunk.
            local_embeddings = self._speaker_embeddings_for_window(
                embedder,
                path,
                global_chunk_intervals,
                window_start=chunk.start_s,
                window_end=chunk.end_s,
            )
            ...
            if chunk.index == 0:
                ...
                # Prefer fresh overlap embeddings when available; fall back to the
                # accumulated gallery for speakers absent from this overlap.
                overlap_end = chunk.start_s + overlap_s
                overlap_gallery = self._speaker_embeddings_for_window(
                    embedder,
                    path,
                    global_intervals,
                    window_start=chunk.start_s,
                ...
                match_gallery = dict(global_gallery)
                match_gallery.update(overlap_gallery)
                mapping, next_global_index = link_local_speakers(
                    ...
                    global_embeddings=match_gallery,
            ...
            for local_id, global_id in mapping.items():
                emb = local_embeddings.get(local_id)
                if emb is not None and global_id not in global_gallery:
                    global_gallery[global_id] = emb
            chunk_results.append(remapped)
            global_intervals.extend(remapped)
            log.info(
                "Diarization chunk %d/%d linked: %d local → %d gallery speakers",
                chunk.index + 1,
                len(chunks),
                len(mapping),
                len(global_gallery),
